"""
Open-set experiment for PAMAP2 dataset: Unknown activity detection
Test activity should be classified as "unseen_activity"
"""
import glob
import os
import random
...
def main():
    ...
    for TEST_ACTIVITY in TEST_ACTIVITIES:
        print(f"\n{'='*100}")
        print(f"PROCESSING TEST ACTIVITY: {TEST_ACTIVITY} - {ACTIVITY_MAPPING.get(TEST_ACTIVITY, '')}")
        print(f"{'='*100}")
        ...
        try:
            ...
            output_dir = f'output/exp1-v2/{str(fewshot)}/{model}/activity_{TEST_ACTIVITY}'
            ...
            for idx, file_path in enumerate(tqdm(all_windows, desc=f"Processing activity {TEST_ACTIVITY}"), 1):
                try:
                    ...
                    print(f"Retrieved {total_hits} documents for activity {activity_id}, subject {subject_id}, window {window_name}")
                    ...
                    retrieved_label_names = [f"{label}:{ACTIVITY_MAPPING.get(int(label), '')}" for label in set(retrieved_labels)]
                    print(f"Retrieved labels: {retrieved_label_names}")

                    retrieved_data = "\n\n".join(sections)

                    # Create classes list with true activity replaced by "unseen_activity"
                    classes_list = []
                    for class_id, class_name in ACTIVITY_MAPPING.items():
                        if class_id == TEST_ACTIVITY:
                            classes_list.append(f"{class_id}. unseen_activity")
                        else:
                    ...
                    # System prompt
                    system_prompt = f"""
You are a multi-class classifier working in the open-set setting.
Your input consists of tri-site sensor readings of an accelerometer, gyroscope and magnetometer placed on subject's chest, right wrist and left ankle that have been summarized into statistical features across temporal segments.
...
1.Given a candidate sample and a set of retrieved labeled samples, decide if the candidate belongs to one of the known classes or should be labeled as unseen_activity.
2.The retrieved samples WILL NOT include the true class of the candidate. They are provided only to help you understand statistical activity patterns.
3.The label unseen_activity is equally valid as any known class. Use it whenever the candidate's statistical patterns do not strongly match a known class.
4.If the candidate does not closely match any known class, classify it as unseen_activity.
5.If uncertain between multiple classes or if similarity is only moderate, prefer unseen_activity instead of forcing a known class.
6.Do not rely only on the retrieved examples being correct—they may be close but not the same activity.
7.Use your knowledge of human activity patterns to inform your decision.

OUTPUT REQUIREMENTS:
1. "label" must be either one of the known classes in {classes_text} or "unseen_activity".
2. "reasoning" should explain how you compared the candidate statistics with known patterns, why you eliminated alternatives, and why the chosen label is the best match..

You must always return a valid JSON object in this format. The JSON must be syntactically valid.
{{
    "label": "<string>",
    "reasoning": "<string>"
}}
...
"""
    ...
    if all_accuracies and all_f1_scores:
        ...
        summary_file = f'output/exp1-v2/{str(fewshot)}/{model}/overall_summary.txt'
        ...
        with open(summary_file, 'w') as f:
            ...
            for i, (acc, f1) in enumerate(zip(all_accuracies, all_f1_scores)):
                ...
                activity_name = ACTIVITY_MAPPING.get(activity_id, '')
